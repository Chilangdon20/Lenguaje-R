...
Un **vector** es una **secuencia ordenada de datos**. `R` dispone de muchos tipos de datos.

En R todos los objetos que hay dentro del vector tiene que ser del mismo tipo.

## Funciones Básicas en R

- ```c()```: Sirve para definir un vector.

- ```scan()```: Sirve para definir un vector.

- ```fix(x)```: Sirve para modificar visualmente el vector $x$.

- ```rep(a,n)```: Sirve para definir un vector constante que contiene el dato $a$ repetido $n$ veces.


```{r}
c(10,12,45)
rep("Fer",6)
names <- c("Fer","Python","R","Ml")
fix(names)
...
- ``seq(a,b,by=d)``: para generar una [progresión aritmética](https://www.superprof.es/apuntes/escolar/matematicas/aritmetica/sucesiones/progresiones-aritmeticas.html) de diferencia $d$ que empieza en $a$ hasta llegar a $b$
